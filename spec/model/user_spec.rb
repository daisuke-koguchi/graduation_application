...
RSpec.describe 'dvevise機能', type: :model do
  describe '新規会員登録機能' do
    ...
    context '姓が20文字以内の場合' do
      it 'バリデーションにかからない' do
        user = FactoryBot.build(:user, first_name: "a" * 20 )
        expect(user).to be_valid
      end
    end
    context '名が空欄の場合' do 
    ...
    context '名が20文字以上の場合' do
      it 'バリデーションに引っかかる' do
        user = FactoryBot.build(:user, last_name: "a" * 21 )
        user.valid?
    ...
    context '名が20文字以内の場合' do
      it 'バリデーションにかからない' do
        user = FactoryBot.build(:user, last_name: "a" * 20 )
        expect(user).to be_valid
      end
    end
    ...
    context '自己紹介が2001字以上場合' do 
      it 'バリデーションに引っかかる' do 
        user = FactoryBot.build(:user, introduction: "a" * 2001 )
        user.valid?
